---
titulo: "Visa Transeúnte — Artistas (versión 2 actualizada)"
descripcion: "Visa Transeúnte — Artistas (versión 2 actualizada). Tasa SNM B/.250 + tasa MITRADEL B/.300 cuando aplique (orientativo, verificar SNM)."
resumen: Versión VIGENTE (preferida por SNM) de la visa para artistas con proyecto cultural específico. Reemplaza a la versión 1. 1 año, prorrogable anualmente hasta 5 veces.
decreto_principal: "Decreto Ejecutivo 320 de 2008, artículo 39 (modificado por Decreto 547/2013)"
categoria_migratoria: "No residente — trabajador artístico con proyecto"
base_legal:
  - Decreto Ley 3 de 22 de febrero de 2008, régimen migratorio de Panamá
  - Decreto Ejecutivo 320 de 2008, artículo 39, régimen de trabajadores eventuales y artísticos
  - Decreto Ejecutivo 547 de 2013, modificación al régimen de artistas
  - Decreto Ejecutivo 26 de 2009, modificaciones al reglamento
dirigido_a:
  - Artista o agrupación artística con proyecto cultural específico en Panamá
  - Trabajador de producción cultural con temporada definida
  - Profesional de las artes escénicas con contrato de presentación en Panamá
requisitos_persona_natural:
  - Poder notariado y solicitud
  - 3 fotografías tamaño carné
  - Pasaporte cotejado (original + copia)
  - Certificado de Antecedentes Penales
  - Certificado de Salud
  - Documentación de la empresa solicitante y del proyecto cultural/artístico específico
requisitos_empresa:
  - Carta de responsabilidad conforme al Art. 89 del Decreto Ley 3/2008
  - Documentación del proyecto cultural/artístico específico
  - Contrato con el artista o agrupación
documentos:
  - Pasaporte (<strong>original cotejado + copia</strong>)
  - Carta de responsabilidad empresarial (Art. 89 Decreto Ley 3/2008)
  - Documentación del proyecto artístico
  - Antecedentes penales
  - Certificado de salud
plazos:
  tramite: "10 a 20 días hábiles, requiere aprobación SNM + MITRADEL"
  vigencia: "1 año, prorrogable anualmente hasta 5 veces"
  renovacion: "Anual, previa verificación de mantenimiento del proyecto artístico que justificó la visa."
honorarios_ref: "Honorarios profesionales según el Acuerdo CNA 609-A/2021. Cotizamos según duración del proyecto y volumen de artistas."
tasas_oficiales: "Tasa SNM B/.250 + tasa MITRADEL B/.300 cuando aplique al perfil."
alertas:
  - "Esta es la VERSIÓN VIGENTE de la visa. El SNM prefiere esta versión sobre la versión 1 (obsoleta). Use esta para todas las solicitudes nuevas."
  - "La empresa contratante está obligada al cumplimiento del Art. 89 del Decreto Ley 3/2008: asume responsabilidad migratoria del trabajador."
  - "Requiere aprobación CONJUNTA del SNM y MITRADEL. No basta con uno solo."
  - "La prórroga anual depende de que el proyecto artístico continúe vigente. Si la temporada o gira terminó, no se renueva la visa y el artista debe regresar a su país."
notas_firma: |
  En DTR Abogados recomendamos SIEMPRE la versión 2 al
  tramitar visas para artistas con proyectos culturales. La razón
  es práctica: la versión 2 tiene la redacción actualizada que el
  SNM prefiere y reduce riesgo de rechazo en consulados del
  exterior.

  Trabajamos con:

  1. **Promotoras de eventos culturales** — conciertos, obras de
     teatro, giras artísticas, festivales internacionales.

  2. **Productoras audiovisuales** — filmaciones temporales con
     equipos artísticos extranjeros.

  3. **Instituciones culturales** — temporadas de orquestas,
     ballet, teatro con artistas internacionales.

  **Casos donde esta visa NO es la vía correcta:** si el artista
  pretende establecerse permanentemente en Panamá, enseñar de
  forma regular o tener una carrera local, debe solicitar visa de
  residente. La visa artística tiene límite de 5 años
  improrrogables.
aprobado_por: "Danna (CLO), 08.08.2026"
revisar_con: "800-Knowledge-PA/810-Migracion/811-Bloque-1-Normativa-y-Requisitos/811.7-Requisitos-SNM/811.7.1-Requisitos-No-Residentes/810.2.1.11-requisitos-visa-transeuntes-artistas-v2-RESUMEN.md"
ultima_actualizacion: 8 de agosto de 2026
---

La versión 2 de la visa transeúnte artistas es la versión VIGENTE
preferida por el SNM. La versión del frontmatter arriba es la canónica;
este cuerpo markdown se mantiene vacío intencionalmente porque toda la
información relevante se renderiza desde los campos del schema.