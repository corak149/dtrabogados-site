---
titulo: Permiso bajo contrato con el gobierno
descripcion: "Permiso de trabajo para extranjeros en empresas CONTRATISTAS del Estado panameno. Distinto del funcionario directo. DE 320/2008 arts. 71-75."
resumen: Permiso de trabajo para extranjeros en empresas CONTRATISTAS del Estado (no funcionarios directos). Distinto del residente temporal por contrato con gobierno (este es para funcionarios directos).
decreto_principal: "Decreto Ejecutivo 320 de 2008, artículos 71-75 (personal de empresas con contrato del Estado)"
categoria_migratoria: "Permiso de trabajo — empresa con contrato del Estado"
base_legal:
  - Decreto Ley 3 de 22 de febrero de 2008, régimen migratorio de Panamá
  - Decreto Ejecutivo 320 de 2008, artículos 71-75
  - Decreto Ejecutivo 6 de 2023, permisos de trabajo MITRADEL
dirigido_a:
  - Personal de empresa contratista del Estado panameño
  - Consultor o técnico extranjero asignado a proyecto estatal
  - Empleado de empresa con contrato de servicios con gobierno
requisitos_persona_natural:
  - Pasaporte vigente
  - Título profesional (cuando aplique)
  - Antecedentes penales apostillados
  - Certificado médico panameño
  - Carta de responsabilidad de la empresa
requisitos_empresa:
  - Empresa debe tener CONTRATO vigente con el Estado
  - Aviso de operación de la empresa
  - Permiso de trabajo vigente
  - Certificación del contrato estatal
documentos:
  - Pasaporte (<strong>original + copia</strong>)
  - Carta de responsabilidad empresarial
  - Contrato con el Estado (referendo o Gaceta Oficial)
  - Aviso de operación
  - Antecedentes penales
plazos:
  tramite: "2 a 4 meses ante el MITRADEL"
  vigencia: "Alineado al contrato estatal"
  renovacion: "Renovable mientras se mantenga el contrato con el Estado."
honorarios_ref: "Honorarios profesionales según el Acuerdo CNA 609-A/2021. Cotizamos según duración del contrato."
tasas_oficiales: "Tasa MITRADEL B/.300 + tasa SNM B/.250 + B/.1,050 en primera solicitud."
alertas:
  - "Esta categoría es para EMPRESAS contratistas del Estado, NO para funcionarios directos. Para funcionarios directos, ver slug residencia-contrato-gobierno."
  - "El contrato con el Estado debe estar vigente y documentado (Gaceta Oficial o refrendo Contraloría)."
  - "El permiso de trabajo del trabajador está supeditado a la vigencia del contrato de la empresa."
  - "Si la empresa pierde el contrato con el Estado, el trabajador debe cambiar de categoría o salir del país."
notas_firma: |
  El permiso bajo contrato con el gobierno es para personal
  de empresas contratistas. En DTR Abogados lo tramitamos
  para:

  1. **Empresas constructoras** — con contratos de obra
     pública.

  2. **Empresas de servicios técnicos** — consultoras,
     auditoras con contratos gubernamentales.

  3. **Empresas de tecnología** — proveedores de servicios
     al Estado.

  **Casos donde esta vía NO es la correcta:** si el
  trabajador es funcionario directo del gobierno (no de
  empresa contratista), debe usar el slug
  residencia-contrato-gobierno.
aprobado_por: "Danna (CLO), 08.08.2026"
revisar_con: "800-Knowledge-PA/810-Migracion/812-Bloque-2-Residencias/812.3-Residente-Temporal/810-temporal-empresas-contrato-estado/"
ultima_actualizacion: 8 de agosto de 2026
---

El permiso bajo contrato con el gobierno es para personal de empresas
contratistas del Estado. La versión del frontmatter arriba es la
canónica; este cuerpo markdown se mantiene vacío intencionalmente
porque toda la información relevante se renderiza desde los campos del
schema.